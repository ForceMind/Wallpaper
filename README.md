...
- 九宫格交互：选中的卡片显示强调边框和勾选标记；点击后显示“正在更换”，成功显示已更换来源，失败显示可重试提示。
- 低内存设计：无图片列表 UI，下载完成后直接写盘并交给系统桌面服务；只保留轻量元数据。
- 稳定性：菜单栏应用禁用 macOS 自动终止，桌面图片设置统一在主线程执行；关键触发、来源失败和成功更换写入统一日志。
...
- **定时触发但没有换图**：查看“控制台”中 `local.wallpaper.app` 的 Wallpaper 日志，确认是来源失败还是桌面设置失败；应用会保留当前壁纸，不主动恢复系统默认。
